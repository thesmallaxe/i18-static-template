...
/**
 * Task: build:gathercontent:project
 *
 * Gets project JSON from GatherContent.
 */
gulp.task('build:gathercontent:project', () => {
  const projectAPICommand =
    `curl https://api.gathercontent.com/items?project_id=${gcProjectID} \
      -u ${gcAuth} \
      -H "${gcVersioning}"`;

  // Call the GatherContent API to get the Project's JSON.
  return gulp.src('')
    .pipe($.run(projectAPICommand))
    .on('error', $.notify.onError())
    .pipe($.rename('project.json'))
    .pipe(gulp.dest(paths.gathercontentDir));
});

/**
 * Task: build:gathercontent:item
 *
 * Gets item JSON from GatherContent.
 */
gulp.task('build:gathercontent:item', (done) => {
  const projectJson = JSON.parse(fs.readFileSync(`${paths.gathercontentDir}/project.json`));

  // Converts JSON data from GatherContent into readable YAML files.
  function convertData() {
    let i = 1;
    // Loop through items from the Project JSON file.
    for (let item of projectJson.data) {
      const itemName = item.name.toLowerCase().replace(/-|\/|\s/g, '_').replace(/\?/g, '');
      // Set variables for the Item JSON file and the Item JSON file that contains the item's file URLs.
      const itemJson = JSON.parse(fs.readFileSync(`${paths.gathercontentDir}/${itemName}.json`));
      const filesJson = JSON.parse(fs.readFileSync(`${paths.gathercontentDir}/${itemName}_files.json`));

      let tabObj = {};

      if (itemJson.data.config.length > 0) {
        // Loop through GatherContent tabs.
        for (let tab of itemJson.data.config) {
          const tabLabel = tab.label.toLowerCase().replace(/-|\/|\s/g, '_').replace(/\?/g, '');
          // Create new element object that will contain data for each tab.
          let elementObj = {};
          // Loop through elements within the GatherContent tab.
          for (let element of tab.elements) {
            const elementLabel = element.label.toLowerCase().replace(/-|\/|\s/g, '_').replace(/\?/g, '');
            // Create a new array of selected values if the element type is a checkbox or radio button.
            if (element.type === 'choice_radio' || element.type === 'choice_checkbox') {
              let optionArr = [];
              ...
              elementObj[elementLabel] = optionArr;
            }
            else if (element.type === 'files') {
              // Match the file field name within the file JSON with the
              // element field name if the element type is 'files' to get
              // the file URL.
              for (let file of filesJson.data) {
                if (file.field === element.name) {
                  elementObj[elementLabel] = file.url;
                  break;
                }
              }
            } else {
              elementObj[elementLabel] = element.value;
            }
          }

          // If more than one tab, structure the YAML file with the tab
          // labels being at the highest level.
          if (itemJson.data.config.length > 1) {
            tabObj[tabLabel] = elementObj;
          // If only one tab, don't show the label of the first tab and
          // set the element labels at the highest level.
          } else {
            tabObj = elementObj;
          }
        }

        // Convert the tabObj into JSON
        const elementJson = JSON.stringify(tabObj);

        // Output the tabObj JSON into a file.
        fs.writeFileSync(`${paths.gathercontentDir}/elements_${itemName}.json`, elementJson);

        // Convert the JSON to YAML and put it in the _data directory.
        gulp.src(`${paths.gathercontentDir}/elements_${itemName}.json`)
          .pipe($.jsonToYaml())
          .pipe($.rename(`${itemName}.yml`))
          .pipe(gulp.dest(paths.gathercontentFinalDir));
      }

      if (i < projectJson.data.length ) {
        i++;
      } else {
        done();
      }
  ...
  function getItem(i) {
    // Using a timeout here isn't ideal, but curl can't seem to
    // handle multiple commands at once.
    setTimeout(function () {
      const item = projectJson.data[i];
      const itemAPICommand =
        `curl https://api.gathercontent.com/items/${item.id} \
          -u ${gcAuth} \
          -H "${gcVersioning}"`;

      const filesAPICommand =
        `curl https://api.gathercontent.com/items/${item.id}/files \
          -u ${gcAuth} \
          -H "${gcVersioning}"`;

      const itemName = item.name.toLowerCase().replace(/-|\/|\s/g, '_').replace(/\?/g, '');

      // Call the GatherContent API to get the Item's JSON.
      gulp.src('')
        .pipe($.run(itemAPICommand))
        .on('error', $.notify.onError())
        .pipe($.rename(`${itemName}.json`))
        .pipe(gulp.dest(paths.gathercontentDir));

      // Call the GatherContent API to get the Item Files' JSON.
      gulp.src('')
        .pipe($.run(filesAPICommand))
        .on('error', $.notify.onError())
        .pipe($.rename(`${itemName}_files.json`))
        .pipe(gulp.dest(paths.gathercontentDir));

      i++;

      // Creates a loop until all Item JSON has been created.
      if (i < projectJson.data.length) {
        getItem(i);
      } else {
        // Converts JSON into YAML on the last iteration.
        setTimeout(function () {
          convertData();
        }, 1000);
      }
...
/**
 * Task: clean:gathercontent
 *
 * Deletes the entire _gathercontent directories.
 */
gulp.task('clean:gathercontent', (callback) => {
  del([paths.gathercontentDir, paths.gathercontentFinalDir]);
  callback();
});

/**
 * Task: build:gathercontent
 *
 * Runs the gathercontentjson build command.
 */
gulp.task('build:gathercontent', (callback) => {
  runSequence('build:gathercontent:project',
    'build:gathercontent:item',
    callback);
});
